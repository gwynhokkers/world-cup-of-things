...
| Unit tests | `bun run test` (Vitest; `tests/` — APIs, comp page helpers, Resend, round voters) |
...
- **No ESLint**: There is no lint script. **Unit tests** use Vitest (`bun run test`). TypeScript checking via `npx nuxi typecheck` has pre-existing type errors that do not block the dev server or build.
